...
class Utilizator:
    ...
    def afiseaza_nivel_acces(self):
        return f"{self.nume} are nivelul de acces {self.nivel_acces}."
...
class UtilizatorManager(Utilizator):
    def __init__(self, nume, nivel_acces="Manager"):
        super().__init__(nume, nivel_acces)
...
class UtilizatorAdmin(Utilizator):
    def __init__(self, nume, nivel_acces="Admin"):
        super().__init__(nume, nivel_acces)
...
class user:
    ...
    @nivel_acces.setter
    def nivel_acces(self, access_lvl):
        self.__nivel_acces = access_lvl
...
class Sistem:
    ...
    def adauga_utilizator(self, utilizator):
        uid = len(self.__utilizatori) + 1
        self.__utilizatori[uid] = utilizator

    def afiseaza_utilizatori(self):
        return [user.nume for user in self.__utilizatori.values()]

    def verifica_nivel_acces(self, user_name):
        for user in self.__utilizatori.values():
            if user.nume == user_name:
                return user.nivel_acces
        return None

    def modifica_name_user(self, uid, new_name):
        if uid in self.__utilizatori:
            self.__utilizatori[uid].nume = new_name

    def modifica_nivel_acces(self, uid, new_access):
        if uid in self.__utilizatori:
            self.__utilizatori[uid].nivel_acces = new_access

    def sterge_utilizator(self, uid):
        if uid in self.__utilizatori:
            del self.__utilizatori[uid]
...
class TechSolutionsApp:
    ...
    def __init__(self, app_version):
        self.versiune_applicatie = app_version
