...
class DataContainer:
    ...
    def __add__(self, other):
        if isinstance(other, DataContainer):
            return DataContainer(self.container + other.container)
...
class DataAnalysis:
    ...
    def __call__(self):
        return [max(container.container) for container in self.containers]
# CODUL TĂU VINE MAI SUS:
...
class DataStatistics:
    ...
    def _get_all_elements(self):
        ...
        for container in self.containers:
            all_elements.extend(container.container)
        return all_elements
    
    def mean(self):
        all_elements = self._get_all_elements()
        return sum(all_elements) / len(all_elements) if all_elements else 0
    ...
    def median(self):
        ...
        if n % 2 == 1:
            ...
            mid1, mid2 = all_elements[n // 2 - 1], all_elements[n // 2]
            return (mid1 + mid2) / 2
    
    def min(self):
        all_elements = self._get_all_elements()
        return min(all_elements) if all_elements else None
...
class DataFilter:
    ...
    def filter_zeros(self):
        all_elements = self._get_all_elements()
        return [el for el in all_elements if el != 0]
    ...
    def filter_under_mean(self):
        all_elements = self._get_all_elements()
        mean_value = sum(all_elements) / len(all_elements) if all_elements else 0
        return [el for el in all_elements if el > mean_value]
